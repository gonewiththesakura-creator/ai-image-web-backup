...
import { rewriteJsonImageRequest } from '../src/media-upstream-proxy.js';
import {
...
const rewrittenImageRequest = JSON.parse(rewriteJsonImageRequest('/v1/images/generations', Buffer.from(JSON.stringify({ model: 'gpt-image-2-all', size: '3840x3840', prompt: 'p' }))).toString('utf8'));
assert.deepEqual(rewrittenImageRequest, { model: 'gpt-image-2', size: '2880x2880', prompt: 'p' }, 'media proxy should map DreamApi all-model and over-budget 4K square to T8-safe values');
assert.deepEqual(JSON.parse(rewriteJsonImageRequest('/v1/images/generations', Buffer.from(JSON.stringify({ model: 'gpt-image-2', size: '2048x2048' }))).toString('utf8')), { model: 'gpt-image-2', size: '2048x2048' }, 'media proxy should keep supported 2K requests unchanged');
